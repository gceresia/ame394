...
function dft(dftTree) {
	for (var i = 0; i < dftTree.length; i++) {
		stack.push([count, dftTree[i].id]);
		if (dftTree[i].children) {
			count++;
			dft(dftTree[i].children);
			count--;
			while (stack.length > 0) {
				DFOrder.push(stack.pop());
			}
		}
	}
}

function quietDft(dftTree) {
	for (var i = 0; i < dftTree.length; i++) {
		stack.push([count, dftTree[i].id]);
		if (dftTree[i].children) {
			count++;
			quietDft(dftTree[i].children);
			count--;
			while (stack.length > 0) {
				quietNodes.push(stack.pop());
			}
		}
	}
}
...
function bft(tree) {
	...
	quietDft(tree);
	var goDeeper = 0;
	for (var i = 0; i<quietNodes.length; i++) {
		var test = quietNodes[i];
		if (goDeeper < test[0]) {
			goDeeper = test[0];
		}
	}
	
	console.log("Max Depth: " + goDeeper);
	for (var i = 0; i <= goDeeper; i++) {
			console.log("Current Depth: " + i);
		for (var j =0; j < quietNodes.length; j++) {
			var test = quietNodes[j];
			if (test[0] == i) {
				BFOrder.push(test[1]);
				console.log(test[1]);
			}
