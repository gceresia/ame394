...
function dft(tree) {
	...
	for (var i = 0; i < tree.length; i++) {
		...
		if (tree[i].children) {
			count++;
			console.log("Found " + tree[i].children.length + " child(s), going down a level");
			dft(tree[i].children);
			count--;
		} else {
			console.log("Leaf!");
...
function quietDft(tree) {
	for (var i = 0; i < tree.length; i++) {
		quietNodes.push([ count, tree[i].id]);
		if (tree[i].children) {
			count++;
			quietDft(tree[i].children);
			count--;
	}
}
}
